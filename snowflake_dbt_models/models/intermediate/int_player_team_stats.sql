...
SUM(PT3_ATT) AS pt3_att,
SUM(PT3_FGM) AS pt3_fgm,
DIV0(SUM(PT3_FGM),SUM(PT3_ATT)) AS pct_3pt,
SUM(FTA) as fta,
SUM(FTM) AS ftm,
DIV0(SUM(FTM),SUM(FTA)) AS ft_pct,
SUM(DRB) AS drb,
SUM(ORB) AS orb,
SUM(TRB) AS trb,
...
FROM {{ ref('player_all_time_stats_source')}}
...
team_stats AS (
    SELECT TEAMID,
    TEAMNAME,
    CASE WHEN DATE_PART(MONTH, GAMEDATE) <= 7 THEN DATE_PART(YEAR,GAMEDATE) -1 ELSE  DATE_PART(YEAR,GAMEDATE) END AS YEAR_TM,
